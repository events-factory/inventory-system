...
class CreateEvent extends CreateRecord
{
    ...
    protected function handleRecordCreation(array $data): Event
    {
        ...
        // Attach items - Optimized to reduce queries and batch inserts if possible
        $itemsToAttach = [];

        if (isset($data['requisition']['items'])) {
            foreach ($data['requisition']['items'] as $itemData) {
                $itemsToAttach[$itemData['item_id']] = ['quantity' => $itemData['pivot']['quantity']];
            }
        } elseif (!empty($this->addedItems)) {
            foreach ($this->addedItems as $itemData) {
                if (isset($itemData['item_id'], $itemData['quantity'])) {
                    $itemsToAttach[$itemData['item_id']] = ['quantity' => $itemData['quantity']];
                }
            }
        }

        if (!empty($itemsToAttach)) {
            $requisition->items()->attach($itemsToAttach);
        }
